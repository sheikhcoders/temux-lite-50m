...
- `scripts/convert_to_safetensors.py` — helper for migrating legacy
  checkpoints.
- `scripts/resolve_conflicts.py` — auto-resolve merge conflicts with selectable
  "ours", "theirs", or combined outputs and optional dry-run mode.

### Resolving merge conflicts automatically
When template updates collide with local changes, run the resolver to pick a
strategy (`ours`, `theirs`, or `combine`) and stage the results in one shot:

```bash
# prefer the current branch when resolving conflicts
python scripts/resolve_conflicts.py --prefer ours

# merge in the incoming branch's version without writing files
python scripts/resolve_conflicts.py --prefer theirs --dry-run

# resolve only specific files while keeping both sides
python scripts/resolve_conflicts.py README.md config.json --prefer combine
```

By default the script inspects `git diff --name-only --diff-filter=U` to locate
unmerged files, rewrites their content without the conflict markers, and stages
the updated files so you can commit immediately.
